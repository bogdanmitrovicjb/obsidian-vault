...
class CreateRubyProjectTest {  
  ...
  private companion object {  
    ...
    @JvmStatic    fun startIde() {  
      context = Starter.newContext(testName = "testCreateRubyProject", ideInfo = IdeProductProvider.RM)  
...
```kotlin
// the following code is a workaround until issue RUBY-34210 is resolved  
jBlist(xQuery { byClass("JBList") }).clickItem("Gem", fullMatch = false)  
if (ui.dialog(xQuery { byTitle("No Ruby Interpreter Specified") }).present()) {  
  button("Ok").click()  
}  
jBlist(xQuery { byClass("JBList") }).clickItem("Empty Project", fullMatch = false)
```